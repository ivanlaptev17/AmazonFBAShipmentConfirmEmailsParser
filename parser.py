...
root.title('Parser')
root.geometry("75x50")
root.iconbitmap("pics/amazon.ico")
...
class Mail:
    ...
    def find(self, ):
        ...
        found = {'Date': self.date, 'City': None,
                 'State': None, 'Quantity': None, 'Item': None}
        ...
        try:
            ...
            raise ParsingError(
                'Unable to find information about State or City')
    ...
    def __str__(self, ):
        """Returns mail (text)"""
        return 'Date: {}\nFrom: {}\nSubject: {}\nBody: \n\n{}'.format(self.date, self.sender_info, self.subject,
                                                                      self.content)
    ...
    with open(file_path, newline='') as csvfile:
        ...
        for line in lines[1:]:
            for i in range(len(content.keys())):
                content[[date, sender_details, subject,
                         body_contents][i]].append(line[i])
...
def main():
    ...
    answer = messagebox.askokcancel(
        'Start Parsing', 'To start parsing you need to choose an output folder')
    if answer != True:
        return None
    ...
    with open(file + '/' + 'parsed.csv', 'w', newline='') as csvfile:
        ...
        for i in range(len(content[date])):
            try:
                ...
                find = mail.find()
                row = find['Date'].split() + [find['City'], find['State'],
                                              find['Quantity'], find['Item']]
                writer.writerow(row)
            except ParsingError as error:
                cant_parse.append([i, str(error)])
                lbl_cant_parse.config(
                    text='Can\'t parse {} letters'.format(len(cant_parse)))

            # Updating widgets
            pb['value'] = round(100 * i / len(content[date]), 2) - 0.01
            text['text'] = 'Progress: {}%'.format(
                round(100 * (i + 1) / len(content[date])))
